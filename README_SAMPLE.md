...
cp .env.example .env
# Edit .env: Add GROQ_API_KEY and configure hyperparameters
...
- `youtubelm_db_dump.sql` - PostgreSQL database dump (62 videos, 1059 chunks)
...
docker exec -i youtubelm-postgres psql -U youtubelm -d youtubelm < youtubelm_db_dump.sql
...
curl -X POST 'http://localhost:6333/collections/youtubelm_transcripts/snapshots/upload' \
...
- **2025-11-17**: Regenerated PostgreSQL dump (`youtubelm_db_dump.sql`) and Qdrant snapshot (`qdrant_snapshot.snapshot`) for sharing with the team.