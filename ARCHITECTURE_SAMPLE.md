...
- **Embeddings**: HuggingFace sentence-transformers (all-MiniLM-L6-v2) with contextual chunking
...
│  React/TS   │     │  ┌────────────────────────────┐  │     │             │
│             │     │  │ API Layer (3 endpoints)    │  │     │ PostgreSQL  │
│ - Chat UI   │     │  ├────────────────────────────┤  │     │ Vector DB   │
│ - Task      │     │  │ Core Modules (3 tasks)     │  │     │             │
│   Switcher  │     │  ├────────────────────────────┤  │     └─────────────┘
...
**PostgreSQL Schema**:
- `videos`: Video metadata (id, title, url, duration, transcript_path)
...
**Qdrant Collection**:
- Collection: `youtubelm_transcripts`
- Vector dimension: 384 (sentence-transformers/all-MiniLM-L6-v2)
- Payload: video_title, video_url, start_time, end_time, text
...
services:
  postgres:
    image: postgres:15
    environment:
      POSTGRES_DB: ${POSTGRES_DB:-youtubelm}
      POSTGRES_USER: ${POSTGRES_USER:-youtubelm}
      POSTGRES_PASSWORD: ${POSTGRES_PASSWORD:-youtubelm}
    ports:
      - "5432:5432"
    volumes:
      - ./postgres_data:/var/lib/postgresql/data
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U ${POSTGRES_USER:-youtubelm} -d ${POSTGRES_DB:-youtubelm}"]
      interval: 10s
      timeout: 5s
      retries: 5
...
2. **Video Summarization**: Coverage, coherence, key point extraction (TODO)
3. **Quiz Generation**: Question quality, difficulty distribution, answer correctness (TODO)
